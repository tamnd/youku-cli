...
describes what users get. Pick a format with `-o`, or let youku choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
youku <command> -o table   # aligned columns for reading
youku <command> -o jsonl   # one JSON object per line, for piping
youku <command> -o json    # a single JSON array
youku <command> -o csv     # spreadsheet friendly
youku <command> -o tsv     # tab-separated
youku <command> -o url     # just the URL column
youku <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
youku <command> --fields id,title,url
```
...
```bash
youku <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
youku <command>            # a table, because this is a terminal
youku <command> | wc -l    # JSONL, because this is a pipe
```
